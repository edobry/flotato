...
export function createEngineImpl(initial: Tuning, audio: AudioContext | null = null): MusicEngine {
  ...
  function start() {
    ...
    // the timeline. It is safe here because the stop above put a tick offset at
    // `now`: from the restart on, the Transport measures ticks as differences
    // of the tempo integral over times after that offset, so the discarded
    // history is never read. The steps stepTempo adds after `now` are the same
    // setValueAtTime events as before (mt#5172).
